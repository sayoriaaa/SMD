...
DATASET = "PrintedDataset"

def parse_arguments():
    parser = argparse.ArgumentParser()
    parser.add_argument('--job_name', default=None, help='job name (default: printdata_00x)')
    parser.add_argument('--denoise_command', 
                        default='l0 -a ', 
                        help='denoise command')
    return parser.parse_args()

def traverse_obj_files():
    gt_path = []
    noise_path = []
    gt_name = []
    noise_name = []

    path = os.path.join(os.getcwd(), "data", DATASET, 'noisy')
    ...
    path = os.path.join(os.getcwd(), "data", DATASET, 'gt')
    file_list = os.listdir(path) 
    for cnt, file_name in enumerate(file_list):
...
if __name__ == "__main__":
    ...
    denoised_files = []
    # find all meshes
    gt_path, gt_name, noise_path, noise_name = traverse_obj_files()
    # create root folder
